...
export async function exportModelToPdf(model, meta) {
  ...
  // Landscape gives the transaction ledger room for its full column set.
  ...
  for (let p = 1; p <= pages; p++) {
    ...
    doc.text(`Page ${p} of ${pages}`, pageW - M, pageH - 16, {
      align: "right",
    });
  }
  ...
  const sections = [
    ...
    },
    {
      ...
      columns: [
        ...
        { header: "Payment", key: "payment", type: "text", align: "left" },
        { header: "Due", key: "due", type: "money", align: "right" },
      ],
      // Full filtered ledger supplied by the page; falls back to the recent
      // preview so the export still works if the complete set is unavailable.
      rows:
...
// =========================================================
// EXPENSES REPORT MODEL
// =========================================================

export function buildExpensesModel(expenses, summary) {
  const s = summary || {};
  const expArray = Array.isArray(expenses) ? expenses : [];

  const kpis = [
    { label: "Total Expenses", value: s.totalAmount || 0, type: "money" },
    { label: "Total Transactions", value: s.totalTransactions || 0, type: "num" },
    { label: "Today's Total", value: s.todayTotal || 0, type: "money" },
    { label: "Month Total", value: s.monthTotal || 0, type: "money" },
    { label: "Average Expense", value: (s.totalTransactions > 0 ? (s.totalAmount || 0) / s.totalTransactions : 0), type: "money" },
  ];
  ...
  const sections = [
    {
      ...
      columns: [
        { header: "Date", key: "date", type: "time", align: "left" },
        { header: "Expense No.", key: "expenseNumber", type: "text", align: "left" },
        { header: "Title", key: "title", type: "text", align: "left" },
        { header: "Category", key: "category", type: "text", align: "left" },
        { header: "Amount", key: "amount", type: "money", align: "right" },
        { header: "Scope", key: "scope", type: "text", align: "left" },
        { header: "Payment Method", key: "paymentMethod", type: "text", align: "left" },
        { header: "Vendor/Recipient", key: "vendor", type: "text", align: "left" },
        { header: "Type", key: "type", type: "text", align: "left" },
        { header: "Source", key: "source", type: "text", align: "left" },
        { header: "Note", key: "note", type: "text", align: "left" },
      ],
      rows: expArray.map(exp => ({
...
export const exportExpensesToPdf = (expenses, summary, meta) =>
  exportModelToPdf(buildExpensesModel(expenses, summary), meta);
export const exportExpensesToExcel = (expenses, summary, meta) =>
  exportModelToExcel(buildExpensesModel(expenses, summary), meta);
export const printExpensesReport = (expenses, summary, meta) =>
  printModel(buildExpensesModel(expenses, summary), meta);
