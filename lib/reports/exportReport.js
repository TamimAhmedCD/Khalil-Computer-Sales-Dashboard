// Report export helpers — PDF (jsPDF), Excel (SheetJS), and Print (isolated iframe).
//
...
// Neutral report model shared by all three exporters.
function buildModel(reportData, meta) {
...
function buildReportHTML(reportData, meta) {
  const { kpis, sections } = buildModel(reportData, meta);
  ...
  const filters = [
    `Period: ${escapeHtml(meta.periodLabel)}`,
    `Seller: ${escapeHtml(meta.sellerLabel)}`,
    `Category: ${escapeHtml(meta.categoryLabel)}`,
    `Payment: ${escapeHtml(meta.paymentLabel)}`,
  ].join(" &nbsp;•&nbsp; ");
...
export function printReport(reportData, meta) {
  const html = buildReportHTML(reportData, meta);
...
export async function exportReportToPdf(reportData, meta) {
  ...
  [
    ...
    `Seller: ${meta.sellerLabel}    Category: ${meta.categoryLabel}    Payment: ${meta.paymentLabel}`,
...
export async function exportReportToExcel(reportData, meta) {
  ...
  const summaryAoa = [
    ...
    [],
    ["Period", meta.periodLabel],
    ["Seller", meta.sellerLabel],
    ["Category", meta.categoryLabel],
    ["Payment Method", meta.paymentLabel],
    ["Generated", meta.generatedAt],
    [],
    ["Metric", "Value"],
